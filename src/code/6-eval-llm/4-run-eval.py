...
LLM_DIR    = config.OUTPUT_DIR / "6-eval-llm" / "anotasi-llm"
OUTPUT_DIR = config.OUTPUT_DIR / "6-eval-llm" / "metrik"
current_eval_name = "konsensus"


def outdir(nama):
    d = OUTPUT_DIR / current_eval_name / nama
...
def eval_ndcg(df, judges):
    ...
    # --- Delta (GPL - Base) nDCG@5 per Kueri (samakan dgn precision/spearman & 5-eval) ---
    ...
    if {"B", "GPL"}.issubset(piv.columns):
        ...
        piv.to_csv(dest / "ndcg_gpl_delta_per_query.csv", index=False, encoding="utf-8-sig")
        ...
        ax_d.set_title(f"Δ(GPL - Base) nDCG@5 per Kueri (LLM: {', '.join(nama_display(j) for j in judges)})")
        fig_d.tight_layout()
        fig_d.savefig(dest / "plot_ndcg_delta.png", dpi=150, bbox_inches="tight")
        plt.close(fig_d)
...
def eval_precision(df, judges):
    ...
    if {"B", "GPL"}.issubset(piv.columns):
        ...
        piv["Δ(GPL-B)"] = (piv["GPL"] - piv["B"]).round(4)
        piv = piv.sort_values("Δ(GPL-B)").reset_index()
        piv.to_csv(dest / "precision_gpl_delta_per_query.csv", index=False, encoding="utf-8-sig")
        fig_d, ax_d = plt.subplots(figsize=(12, max(5, 0.25 * len(piv))))
        piv["Q_Label"] = piv.apply(lambda r: f"{str(r['kueri'])[:50]}...", axis=1)
        sns.barplot(data=piv, x="Δ(GPL-B)", y="Q_Label", hue="Family", dodge=True, ax=ax_d)
        ax_d.axvline(0, color='black', linewidth=1)
        ax_d.set_title(f"Δ(GPL - Base) P@{K} per Kueri (LLM: {', '.join(nama_display(j) for j in judges)})")
        fig_d.tight_layout()
        fig_d.savefig(dest / "plot_precision_delta.png", dpi=150, bbox_inches="tight")
        plt.close(fig_d)
...
def eval_spearman(df, judges):
    ...
    if {"B", "GPL"}.issubset(piv.columns):
        ...
        piv["Δ(GPL-B)"] = (piv["GPL"] - piv["B"]).round(4)
        piv = piv.sort_values("Δ(GPL-B)").reset_index()
        piv.to_csv(dest / "spearman_gpl_delta_per_query.csv", index=False, encoding="utf-8-sig")
        fig_d, ax_d = plt.subplots(figsize=(12, max(5, 0.25 * len(piv))))
        piv["Q_Label"] = piv.apply(lambda r: f"{str(r['kueri'])[:50]}...", axis=1)
        sns.barplot(data=piv, x="Δ(GPL-B)", y="Q_Label", hue="Family", dodge=True, ax=ax_d)
        ax_d.axvline(0, color='black', linewidth=1)
        ax_d.set_title(f"Δ(GPL - Base) Spearman per Kueri (LLM: {', '.join(nama_display(j) for j in judges)})")
        fig_d.tight_layout()
        fig_d.savefig(dest / "plot_spearman_delta.png", dpi=150, bbox_inches="tight")
        plt.close(fig_d)
...
def eval_iaa():
    ...
    df, daftar_judge = load_annotations(consensus=False, anotasi_dir=LLM_DIR)
    if df is None:
        return
...
def main():
    global current_eval_name
    pilihan = sys.argv[1].lower() if len(sys.argv) > 1 else "all"

    print(f"{'='*60}")
    print(f"  EVAL LLM JUDGE — sumber: {LLM_DIR}")
    print(f"  Output: {OUTPUT_DIR}")
    print(f"{'='*60}")

    df, judges = load_annotations(anotasi_dir=LLM_DIR)
    if df is None:
    ...
    df_indiv, _ = load_annotations(anotasi_dir=LLM_DIR, consensus=False, verbose=False)
