...
def main():
    ...
    pids = list(pid2text)
    queries = list(_gpl.read_jsonl(_gpl.QUERIES)) if _gpl.QUERIES.exists() else []
    if not queries:
        print("queries.jsonl kosong. Jalankan 1-query-gen.py dulu.")
        return
    rn = config.GPL_RETRIEVER
    print(f"[hard-neg] {len(queries):,} kueri, {len(pids):,} pasase, retriever={rn}")

    device = "cuda" if torch.cuda.is_available() else "cpu"
    model = SentenceTransformer(config.resolve_model(rn), trust_remote_code=True, device=device)

    corpus = [_gpl.with_prefix(rn, pid2text[p], "passage") for p in pids]
    emb = model.encode(corpus, batch_size=config.GPL_BATCH_SIZE, convert_to_tensor=True,
                       show_progress_bar=True, normalize_embeddings=True)
    qtexts = [_gpl.with_prefix(rn, q["query"], "query") for q in queries]
    ...
    for q, h in zip(queries, hits):
        pos = q["pid"]
        cands = [pids[x["corpus_id"]] for x in h if pids[x["corpus_id"]] != pos]
