"""
3-panjang.py — Panjang chunk dalam TOKEN per tokenizer model (justifikasi max_seq).

POST-chunking: mengukur chunk FINAL (akurat untuk memilih max_seq_length).
Sumber: src/output/3-praproses (via _load) | Output: src/output/3-praproses/3-panjang.txt
Usage : python src/code/3-praproses/3-panjang.py
"""
...
OUT_TXT = config.PRAPROSES_DIR / "3-panjang.txt"
...
def main():
    ...
    print("=== PRAPROSES 3 — PANJANG TOKEN CHUNK FINAL ===\n")
    ...
    print("-" * len(hdr))
    print(f"\nGPL_MAX_SEQ saat ini = {config.GPL_MAX_SEQ} (token).")
    print(f"\n[INFO] {OUT_TXT}")
